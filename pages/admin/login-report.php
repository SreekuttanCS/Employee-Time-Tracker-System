...
function convertToTimeFormat($time) {
    if (strpos($time, ':') === false) {
        return '00:00:00'; // If the time is invalid
    }

    $time_parts = explode(':', $time);
    return sprintf('%02d:%02d:%02d', $time_parts[0], $time_parts[1], $time_parts[2]);
}

// Function to calculate overtime (if any)
function calculateOvertime($workHour, $totalWorkHours) {
    // Convert both work hours and total work hours to seconds
    $workHourInSeconds = convertToSeconds($workHour);
    $totalWorkHoursInSeconds = convertToSeconds($totalWorkHours);

    // If workHour exceeds totalWorkHours, calculate overtime
    if ($workHourInSeconds > $totalWorkHoursInSeconds) {
        // Calculate the overtime (difference between work hours and total work hours)
        $overtimeDuration = $workHourInSeconds - $totalWorkHoursInSeconds;

        return convertToTimeFormatFromSeconds($overtimeDuration);
    }
...
function convertToTimeFormatFromSeconds($seconds) {
                            ...
                            <?php while ($row = mysqli_fetch_assoc($result)) : ?>
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <span class="status <?php echo strtolower($row['status'] ?? 'offline'); ?>">
